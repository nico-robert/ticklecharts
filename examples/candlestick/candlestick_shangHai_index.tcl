...
proc calculateMA {dayCount data} {
    ...
    for {set i 0} {$i < [llength $data]} {incr i} {
        ...
        set sum 0
        for {set j 0} {$j < $dayCount} {incr j} {
            set sum [expr {$sum + [lrange [lindex $data [expr {$i -$j}]] 1 1]}]
        }

        lappend result [expr {$sum / double($dayCount)}]
...
for {set i 0} {$i < [llength $data0]} {incr i} {
    lappend categoryData [lindex [lindex $data0 $i] 0]
    lappend values [lrange [lindex $data0 $i] 1 end]
}
...
$chart SetOptions -title    {text "上证指数" left 0} \
                  -tooltip  {trigger "axis" axisPointer {type "cross"}} \
                  -legend   [list data [list {"日K" "MA5" "MA10" "MA20" "MA30"}]] \
                  -grid     {left "10%" right "10%" bottom "15%"} \
                  -dataZoom {{type "inside" start 50 end 100} {type "slider" show "True" top "90%" start 50 end 100}}

$chart Xaxis -data [list $categoryData] -boundaryGap "False" -axisLine {onZero "False"} \
...
$chart Add "candlestickSeries" -name "日K" -data $values -itemStyle [list color $upColor color0 $downColor borderColor $upBorderColor borderColor0 $downBorderColor] \
                            ...
                            -markLine [list \
                                        ...
                                        data {
                                            ...
                                            objectItem {name "min line on close" type "min" valueDim "close"}
                                            objectItem {name "max line on close" type "max" valueDim "close"}
                                            } \
                                        ]
